...
**Chrome:** `GtkHeaderBar` title widget/pack/show-title-buttons; `GtkWindow` `set_titlebar` / `get_titlebar`; `GtkMenuButton` label/popover/popup; `GtkPopover` child/popup/autohide; `GtkScrolledWindow` child/policy/min content; `GtkNotebook` pages; `GtkStack` add/visible child; `GtkPaned` start/end/position; `GtkGrid` attach/spacing; `GtkCenterBox` start/center/end; `GtkOverlay` child/overlays; `GtkListBox` append/select/row; `GtkListView` + `GtkStringList` + `GtkSingleSelection` + `GtkListItem` factory/setup/bind.
